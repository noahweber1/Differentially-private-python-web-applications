...
def startup():
    global roadtrip_df, naive_bayes

    # prepare golf data set
    roadtrip_df = BuildRoadTripDataSet()

    # create dummy features 
    roadtrip_df_ready = PrepareDataForModel(roadtrip_df, target_columns=['Outlook', 'Temperature_Nominal', 'Humidity_Nominal'])
    roadtrip_df_ready = roadtrip_df_ready.dropna() 

    # build feature set
    features = [feat for feat in list(roadtrip_df_ready) if feat != 'Road_Trip']

    # run bayesian model 
    naive_bayes = GaussianNB()
    naive_bayes.fit(roadtrip_df_ready[features], np.ravel(roadtrip_df_ready[['Road_Trip']]))
...
def RoadTrip():
    ...
    if request.method == 'POST':
        ...
        openweathermap_url = "http://api.openweathermap.org/data/2.5/forecast?q=" + selected_location + "&mode=json&APPID=7940fa88f265d6334c84e901f9555e34"
